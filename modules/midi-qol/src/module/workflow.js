...
export class Workflow {
	...
	async _next(newState, undoData = {}) {
		...
		switch (newState) {
			...
			case WORKFLOWSTATES.VALIDATEROLL:
				...
				if (checkRule("incapacitated") && checkIncapacitated(this.actor, this.item, null))
			...
			case WORKFLOWSTATES.WAITFORATTACKROLL:
				...
				if (this.rollOptions?.fastForwardSet)
					this.autoRollAttack = true;
				if (this.rollOptions.rollToggle)
					this.autoRollAttack = !this.autoRollAttack;
			...
			case WORKFLOWSTATES.ATTACKROLLCOMPLETE:
				...
				if (checkRule("removeHiddenInvis"))
					await removeHiddenInvis.bind(this)();
			...
			case WORKFLOWSTATES.WAITFORDAMAGEROLL:
				...
				if (this.shouldRollDamage) {
					...
					if (chatMessage) {
						...
						if (this.rollOptions.fastForwardDamage)
			...
			case WORKFLOWSTATES.DAMAGEROLLCOMPLETE:
				...
				if (this.item?.data.data.actionType === "heal" && !Object.keys(getSystemCONFIG().healingTypes).includes(this.defaultDamageType ?? ""))
			...
			case WORKFLOWSTATES.APPLYDYNAMICEFFECTS:
				...
				for (let theItem of items) {
					...
					if (hasItemTargetEffects || ceTargetEffect) {
						...
						anyActivationTrue = true; // There were no target effects at all - special case treat this.
	...
	async checkAttackAdvantage() {
		...
		if (advantage) {
			...
			if (advantage.attack?.all)
				this.attackAdvAttribution["ADV:attack.all"] = true;
			if (advantage.attack && advantage.attack[actType])
		...
		if (disadvantage) {
			...
			if (disadvantage.attack?.all)
				this.attackAdvAttribution["DIS:attack.all"] = true;
			if (disadvantage.attack && disadvantage.attack[actType])
	...
	async checkFlankingAdvantage() {
		...
		if (this.item && !(["mwak", "msak", "mpak"].includes(this.item?.data.data.actionType)))
			return false;
	...
	*/
	async checkSaves(whisper = false, simulate = false) {
		...
		try {
			...
			if (this.token && this.token.data?.disposition)
				actorDisposition = this.token.data.disposition;
			...
			for (let target of allHitTargets) {
				...
				if (isFriendly && this.saveItem.data.data.description.value.includes(i18n("midi-qol.autoFailFriendly"))) {
					...
					if (debugEnabled > 0)
						warn(`Player ${player?.name} controls actor ${target.actor.name} - requesting ${getSystemCONFIG().abilities[this.saveItem.data.data.save.ability]} save`);
					promises.push(new Promise((resolve) => {
	...
	async checkHits() {
		...
		for (let targetToken of this.targets) {
			...
			if (targetActor.type === "vehicle" && !wjVehicle) {
				...
				if (inMotion)
					targetAC = Number.parseInt(targetActor.data.data.attributes.ac.flat ?? 10);
				else
					targetAC = Number.parseInt(targetActor.data.data.attributes.ac.motionless ?? 10);
			}
			...
			if (this.useActiveDefence) {
				...
				if (!this.isFumble) {
					...
					if (checkRule("challengeModeArmor"))
						...
						isHit = attackTotal >= targetAC || this.isCritical;
					if (targetEC)
						isHitEC = checkRule("challengeModeArmor") && attackTotal <= targetAC && attackTotal >= targetEC;
