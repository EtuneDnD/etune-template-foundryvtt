...
import { actionQueue, addConvenientEffectsChange, addCubChange, addTokenMagicChange, applyDaeEffects, daeMacro, daeSystemClass, fetchParams, libWrapper, noDupDamageMacro, prepareLastArgData, removeConvenientEffectsChange, removeCubChange, removeTokenMagicChange } from "../dae.js";
import { cleanArmorWorld, cleanDAEArmorWorld } from "../migration.js";
import { applyBaseActiveEffects, ValidSpec, wildcardEffects } from "./DAESystem.js";
var displayTraits;
...
export class DAESystemDND5E extends CONFIG.DAE.systemClass {
    static modifyBaseValues(actorType, baseValues, characterSpec) {
        ...
        baseValues["data.traits.di.value"] = "";
        baseValues["data.traits.di.custom"] = "";
        baseValues["data.traits.dr.all"] = false;
        baseValues["data.traits.dr.value"] = "";
        baseValues["data.traits.dr.custom"] = "";
        baseValues["data.traits.dv.all"] = false;
        baseValues["data.traits.dv.value"] = "";
        baseValues["data.traits.dv.custom"] = "";
        baseValues["data.traits.ci.all"] = false;
        baseValues["data.traits.ci.value"] = "";
        baseValues["data.traits.ci.custom"] = "";
        baseValues["data.traits.size"] = "";
    ...
    static modifySpecials(actorType, specials, characterSpec) {
        ...
        if (actorType === "vehicle") {
            specials["data.attributes.ac.motionless"] = [0, -1];
            specials["data.attributes.ac.flat"] = [0, -1];
        }
        else {
        ...
        for (let abl of Object.keys(CONFIG.DND5E.abilities)) {
            specials["data.abilities.${abl}.dc"] = [0, ACTIVE_EFFECT_MODES.CUSTOM];
        }
        ...
        */
        ...
        if (characterSpec.data.resources) {
            ...
            if (characterSpec.data.spells) {
                for (let spellSpec of Object.keys(characterSpec.data.spells))
                    specials[`data.spells.${spellSpec}.max`] = [0, -1];
            }
            /*/
            if (game.modules.get("resourcesplus")?.active) {
    ...
    static modifyValidSpec(spec, validSpec) {
        //@ts-ignore
        const ACTIVE_EFFECT_MODES = CONST.ACTIVE_EFFECT_MODES;
        // TODO move this into a system specfic bit of code.
    ...
    static initActions() {
        ...
        if (game.modules.get("dnd5e-custom-skills")?.active) {
            wildcardEffects.push(/data\.skills\..*\.value/);
            wildcardEffects.push(/data\.skills\..*\.bonuses/);
            wildcardEffects.push(/data\.abilities\..*\.bonuses/);
        }
    ...
    static initSystemData() {
        ...
        daeSystemClass.daeActionTypeKeys = Object.keys(CONFIG.DND5E.itemActionTypes);
    }
    static effectDisabled(actor, effect, itemData = null) {
        effect.determineSuppression();
        const disabled = effect.data.disabled || effect.isSuppressed;
        return disabled;
    }
    // For DAE Editor
    ...
    static daeCustomEffect(actor, change) {
        ...
        if (change.key.includes(`flags.${game.system.id}`)) {
            //@ts-ignore safeEval
            const value = new Boolean(Roll.safeEval(change.value)); // ["1", "true", 1, true].includes(change.value);
        ...
        if (change.key.startsWith("data.abilities") && (change.key.endsWith("bonuses.save") || change.key.endsWith("bonuses.check"))) {
            value = change.value;
            if (!current)
        ...
        */
        switch (change.key) {
            ...
            case "data.traits.sdv.all":
                ...
                if (game.system.id === "dnd5e")
                    setProperty(actor.data, key, Object.keys(CONFIG.DND5E.damageResistanceTypes).filter(k => !["healing", "temphp"].includes(k)));
                else
                    setProperty(actor.data, key, Object.keys(CONFIG.DND5E.damageResistanceTypes).filter(k => !["healing", "temphp"].includes(k)));
                return true;
...
async function toggleDisabledEffect(item, updates) {
    ...
    for (let aef of item.parent.effects) { // remove all transferred effects for the item
        ...
        if (isTransfer && aef.data.origin === item.uuid && aef.data.disabled !== disabled) {
            wasChanged = true;
...
function determineSuppression() {
    ...
    if (this.data.disabled || (this.parent.documentName !== "Actor"))
        return;
    if (this.data.origin) {
        const originParts = this.data.origin.split(".");
    ...
    if (this.parent) {
        const ci = this.parent.data.data.traits?.ci?.value;
        const statusId = (this.data.label ?? "no effect").toLocaleLowerCase();
        const ciSuppressed = (ci?.length && ci.some(c => statusId == c));
        this.data.disabled = Boolean(ciSuppressed);
    }
}
function preUpdateItemHook(candidate, updates, options, user) {
    ...
    if (!(actor instanceof Actor))
        return true;
    if (updates.data?.equipped === undefined && !updates.data?.attunement === undefined)
        return true;
    try {
        const wasSuppressed = candidate.areEffectsSuppressed;
        candidate.data.data.equipped = updates.data?.equipped ?? candidate.data.data.equipped;
        candidate.data.data.attunement = updates.data?.attunement ?? candidate.data.data.attunement;
        const isSuppressed = candidate.areEffectsSuppressed;
        ...
        for (let effect of candidate.effects) {
            if (!effect.data.flags.dae.transfer || effect.data.origin !== candidate.uuid)
                continue;
            for (let change of effect.data.changes) {
                if (isSuppressed) {
                    switch (change.key) {
                        case "macro.CE":
                            ...
                            removeConvenientEffectsChange(change.value, actor.uuid, effect.data.origin, lastArg);
                            break;
                        case "macro.CUB":
            ...
            if (effect.data.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                actionQueue.add(daeMacro, isSuppressed ? "off" : "on", actor, effect.data, actor.getRollData());
        }
    }
    finally {
        return true;
