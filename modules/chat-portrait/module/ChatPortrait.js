...
export class ChatPortrait {
    ...
    static loadImagePathForChatMessage(html, speakerInfo) {
        ...
        if (speaker) {
            ...
            if (speaker.token) {
                tokenDocument = ChatPortrait.getTokenFromScene(speaker.scene, speaker.token);
                if (!tokenDocument) {
                    tokenDocument = ChatPortrait.getTokenFromId(speaker.token);
                }
                if (!tokenDocument && speaker.actor) {
                    tokenDocument = ChatPortrait.getTokenFromActor(speaker.actor);
                }
    ...
    static loadImagePathForCombatTracker(tokenID, actorID, userID, sceneID, isOwnedFromPLayer) {
        ...
        let tokenDocumentData;
        if (tokenID) {
            tokenDocument = ChatPortrait.getTokenFromScene(sceneID, tokenID);
            if (!tokenDocument) {
                tokenDocument = ChatPortrait.getTokenFromId(tokenID);
            }
            if (!tokenDocument && actorID) {
                tokenDocument = ChatPortrait.getTokenFromActor(actorID);
            }
            // THIS PIECE OF CODE IS PROBABLY NOT NECESSARY ANYMORE ??
            if (!tokenDocument) {
                try {
