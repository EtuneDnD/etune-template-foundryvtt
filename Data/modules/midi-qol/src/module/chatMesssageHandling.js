...
import { createDamageList, getTraitMult, calculateDamage, MQfromUuid, playerFor, playerForActor } from "./utils.js";
...
export let processCreateBetterRollsMessage = (message, user) => {
	...
	//@ts-ignore
	if (debugEnabled > 1)
		debug("process preCreateBetterRollsCard ", message.data, installedModules["betterrolls5e"], message.data.content?.startsWith('<div class="dnd5e red-full chat-card"'));
	let actorId = brFlags.actorId;
	let tokenId = brFlags.tokenId;
	if (tokenId && !tokenId.startsWith("Scene")) { // remove when BR passes a uuid instead of constructed id.
		const parts = tokenId.split(".");
...
export let hideStuffHandler = (message, html, data) => {
	...
	if (getProperty(message.data, "flags.monks-tokenbar"))
		return;
