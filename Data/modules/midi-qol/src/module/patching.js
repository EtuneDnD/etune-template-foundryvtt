	...
	try {
		...
		if (!Array.isArray(macroChanges.items)) { // we have an update from editing the macro changes
			for (let keyString in macroChanges.items) {
				...
				if (macroChanges.items[key].macroName)
					macroParts.items[key].macroName = macroChanges.items[key].macroName;
				if (macroChanges.items[key].option)
					macroParts.items[key].option = macroChanges.items[key].option;
			}
		}
		...
		if ((this.parent instanceof CONFIG.Actor.documentClass)) {
			...
			if (isConcentration)
				...
				if (concentrationData) {
					const concentrationTargets = concentrationData.targets.filter(target => target.tokenUuid !== effect.parent.uuid && target.actorUuid !== effect.parent.uuid);
