...
Hooks.on('init', () => {
    class FogManagerLayer extends (game.release.generation === 9 ? CanvasLayer : InteractionLayer) {
         ...
         */
        async draw() { // V10 should have '_draw' and no super, but v9 doesn't like this
            await super.draw();
            //this.visible = false;
            if (!this.#initialized) {
            ...
            this.renderStack(canvas.scene.getFlag(this.layername, "history"), 0);
        ...
        static getMaskTexture() { // Create the mask elements
            ...
            const tex = PIXI.RenderTexture.create(res);
            return tex;
        }
        ...
        getUserSetting(name) {
            ...
            if (name == "brushOpacity") {
                if (game.user.getFlag(this.layername, "brushMode") == 1)
                    return 0x000000
                else
                    return 0xFFFFFF
         ...
         */
        async renderStack(history = canvas.scene.getFlag(this.layername, "history"), start = this.pointer, stop = canvas.scene.getFlag(this.layername, "history.pointer"),) { // If history is blank, do nothing
        ...
        async receive_fog() {
            ...
            console.log(`Call to doSomething took ${endTime - startTime} milliseconds`) // TODO Fix
        }
    ...
    */
            ...
            for (let i = 0; i < game.users.size; i++) {
                ...
                if (exploration) {
                    fog_to_delete.push(exploration?.data._id ?? exploration._id)
                }
        ...
        setActiveTool(tool) {
            if (game.release.generation === 9) {
                var {grid, gridType} = canvas.scene.data;
            } else {
                var gridType = canvas.scene.grid.type
                var grid = canvas.scene.grid.size
            }
...
Hooks.on("createFogExploration", async function (document, options, userId) {
    fogManagerLog.info("UPDATE", document)
    if (game.release.generation === 9) {
