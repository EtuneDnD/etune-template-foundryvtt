...
export const WallsReach = {
    globalInteractionDistance: function (selectedToken, wall, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(WallsReach.getWallsCenter(wall), selectedToken, WallDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(WallsReach.getWallsCenter(wall), selectedToken, WallDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.wallsNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
