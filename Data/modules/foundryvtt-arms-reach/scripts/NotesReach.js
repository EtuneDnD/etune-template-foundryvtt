...
export const NotesReach = {
    globalInteractionDistance: function (selectedToken, note, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(NotesReach.getNotesCenter(note), selectedToken, NoteDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(NotesReach.getNotesCenter(note), selectedToken, NoteDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.notesNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
