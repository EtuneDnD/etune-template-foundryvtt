...
export class TokenInfo {
    ...
    getFlag(flagName, dflt = undefined) {
        ...
        return (this.token.document.getFlag(CONSTANTS.MODULE_NAME, flagName) ??
            baseActor.getFlag(CONSTANTS.MODULE_NAME, flagName) ??
            dflt);
    }
    get weaponRange() {
        return (this.token.document.getFlag(CONSTANTS.MODULE_NAME, CONSTANTS.FLAG_NAMES.WEAPON_RANGE) ||
    ...
    async setIgnoreDifficultTerrain(isIgnore, updateActor = false) {
        await this.token.document.setFlag(CONSTANTS.MODULE_NAME, CONSTANTS.FLAG_NAMES.IGNORE_DIFFICULT_TERRAIN, updateActor);
    }
    get speed() {
        ...
        const speedAttrPathSetting = game.settings.get(CONSTANTS.MODULE_NAME, "speed-attr-path");
    ...
    getSpeedFromAttributes() {
        ...
        if (game.system.id === "pf1" || game.system.id === "D35E") {
            ...
            otherSpeeds = Object.entries(actorAttrs.movement)
