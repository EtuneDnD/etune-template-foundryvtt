import { calculateGridDistance, canvasGridSize, canvasTokensGet, getCombatantToken, getCombatantTokenDisposition, getCurrentToken, safeDestroy, } from "./utility.js";
...
function getDiagonalDelta() {
    ...
    if (diagonalsSetting === CONSTANTS.diagonals.FIVE_TEN_FIVE ||
        diagonalsSetting === CONSTANTS.diagonals.TEN_FIVE_TEN) {
        return 0.5;
    }
        ...
        for (const combatant of game.combat?.combatants) {
            ...
            if (getCombatantTokenDisposition(combatantToken) !== myDisposition) {
                if (combatantToken.visible && !combatant.isDefeated) {
                    ...
                    tokenOverlay.drawCircle(widthArea / 2, heightArea / 2, Math.pow(Math.pow(widthArea / 2, 2) + Math.pow(heightArea / 2, 2), 0.5));
