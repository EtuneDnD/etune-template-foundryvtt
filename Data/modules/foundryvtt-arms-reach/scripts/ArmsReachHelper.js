import { getElevationPlaceableObject, warn } from "./lib/lib.js";
 ...
 * @returns
 */
export const computeDistanceBetweenCoordinates = function (placeable, selectedToken, documentName, useGrids) {
    ...
    //@ts-ignore
    const unitSize = canvas.dimensions.distance; //<number>canvas.grid?.grid?.options.dimensions.distance;
    if (useGrids) {
...
export const getFirstPlayerTokenSelected = function () {
    ...
    if (selectedTokens[0] &&
        game.settings.get(CONSTANTS.MODULE_NAME, "enableInteractionForTokenOwnedByUser")) {
...
export const getFirstPlayerTokenNo = function (noToken) {
    ...
    if (!token) {
        if (game.settings.get(CONSTANTS.MODULE_NAME, "useOwnedTokenIfNoTokenIsSelected")) {
            if (!controlled.length || controlled.length === 0) {
                // If no token is selected use the token of the users character
                token = canvas.tokens?.placeables.find((token) => 
                //@ts-ignore
...
export const checkTaggerForAmrsreachForToken = function (placeable) {
...
export const checkTaggerForAmrsreachForNote = function (placeable) {
...
export const checkTaggerForAmrsreachForLight = function (placeable) {
...
export const checkTaggerForAmrsreachForSound = function (placeable) {
...
export const checkTaggerForAmrsreachForDrawing = function (placeable) {
...
export const checkTaggerForAmrsreachForTile = function (placeable) {
...
export const checkTaggerForAmrsreachForWall = function (placeable) {
...
// ============================================================================================
function distance_between_token_rect(p1, p2) {
    //@ts-ignore
...
function grids_between_token_and_placeable(token, b) {
    return Math.floor(distance_between_token_rect(token, b) / canvas.grid?.size) + 1;
}
function units_between_token_and_placeable(token, b) {
...
export const globalInteractionDistanceUniversal = function (placeableObjectSource, placeableObjectTarget, useGrids) {
    const placeableSource = getPlaceableCenter(placeableObjectSource);
    placeableSource.documentName = placeableObjectSource.document.documentName;
    const placeableTarget = getPlaceableCenter(placeableObjectTarget);
...
function grids_between_placeable_and_placeable(a, b) {
    return Math.floor(distance_between_placeable_rect(a, b) / canvas.grid?.size) + 1;
}
function distance_between_placeable_rect(p1, p2) {
    const x1 = p1.x;
    const y1 = p1.y;
    const x1b = p1.x + p1.w;
    const y1b = p1.y + p1.h;
...
function units_between_placeable_and_placeable(a, b) {
    ...
    let dist = (getUnitTokenDistUniversal(a, b) * canvas.dimensions?.size) / canvas.dimensions?.distance -
        tokensSizeAdjust;
    // return dist <= range;
    const unitSize = canvas.dimensions?.distance || 5;
    const unitGridSize = canvas.grid?.size || 50;
...
function getUnitTokenDistUniversal(a, b) {
...
// ================================================
function getUnitTokenDist(token, placeableObjectTarget) {
    const unitsToPixel = canvas.dimensions?.size / canvas.dimensions?.distance;
 ...
 **/
export function isTokenInRange(objectSource, objectTarget) {
    if (game.modules.get("levels")?.active) {
        let rangeTop = objectTarget.document.getFlag("levels", "rangeTop");
        let rangeBottom = objectTarget.document.getFlag("levels", "rangeBottom");
        if (!rangeTop && rangeTop !== 0)
            rangeTop = Infinity;
