...
import { computeDistanceBetweenCoordinates, getFirstPlayerToken, getPlaceableDoorCenter, getTokenCenter, isFocusOnCanvas, interactionFailNotification, } from "./ArmsReachHelper.js";
...
export const DoorsReach = {
    init: function () {
        ...
        if (game.settings.get(CONSTANTS.MODULE_NAME, "hotkeyDoorInteractionDelay") > 0) {
            document.addEventListener("keyup", (evt) => {
    ...
    globalInteractionDistance: function (selectedToken, doorControl, isRightHanler, maxDistance, useGrid, userId) {
                ...
                if (!selectedToken) {
                    ...
                    if (!jumDefaultComputation) {
                        ...
                        if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 ||
                            ...
                            // );
                            const dist = computeDistanceBetweenCoordinates(DoorsReach.getDoorCenter(doorControl), selectedToken, WallDocument.documentName, true);
                            ...
                            const dist = computeDistanceBetweenCoordinates(DoorsReach.getDoorCenter(doorControl), selectedToken, WallDocument.documentName, false);
                            isNotNearEnough = dist > maxDist;
                        }
                    }
                    ...
                    if (isNotNearEnough) {
                        ...
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.doorNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
    ...
    ifStuckInteract: function (key, offsetx, offsety) {
        ...
        if ((Date.now() - ArmsReachVariables.lastData[key]) / 1000 >
            game.settings.get(CONSTANTS.MODULE_NAME, "hotkeyDoorInteractionDelay")) {
    ...
    interactWithNearestDoor: function (token, offsetx = 0, offsety = 0) {
        ...
        // OLD SETTING
        let globalMaxDistance = game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance");
        if (globalMaxDistance <= 0) {
            globalMaxDistance = game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionMeasurement");
        }
        if (globalMaxDistance > 0) {
            ...
            // if (maxDistance <= 0) {
            if (game.settings.get(CONSTANTS.MODULE_NAME, "doorInteractionMeasurement") > 0) {
                maxDistance = game.settings.get(CONSTANTS.MODULE_NAME, "doorInteractionMeasurement");
            }
            // }
        }
        // Shortest dist
        ...
        for (let i = 0; i < game.scenes?.current?.walls.contents.length; i++) {
            ...
            //@ts-ignore
            if (wall.door > 0) {
                const door = canvas.controls?.doors?.children.find((x) => {
                    //@ts-ignore
                    return x.wall.id === wall.id;
                });
                // if (!door.visible) {
                //   continue;
                // }
        ...
        if (closestDoor) {
            ...
            if (tokenName) {
                interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.doorNotFoundInReachFor`, { tokenName: tokenName }));
            }
            else {
     ...
     * Get dorr center
     */
    getDoorCenter: function (doorCoontrol) {
...
ArmsReachVariables.grace_distance = 2.5;
