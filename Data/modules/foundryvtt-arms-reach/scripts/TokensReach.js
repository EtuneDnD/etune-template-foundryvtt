...
export const TokensReach = {
    globalInteractionDistance: function (selectedToken, token, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(TokensReach.getTokensCenter(token), selectedToken, TokenDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(TokensReach.getTokensCenter(token), selectedToken, TokenDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.tokensNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
