...
export const LightsReach = {
    globalInteractionDistance: function (selectedToken, light, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(LightsReach.getLightsCenter(light), selectedToken, AmbientLightDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(LightsReach.getLightsCenter(light), selectedToken, AmbientLightDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.lightsNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
    ...
    getLightsCenter: function (light) {
