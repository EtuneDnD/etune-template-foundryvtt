...
export const TemplatesReach = {
    globalInteractionDistance: function (selectedToken, template, maxDistance, useGrid, userId) {
        ...
        if ((!canvas.tokens?.controlled && game.user?.isGM) ||
            ...
            (!game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistanceForGMOnTemplates") &&
                game.user?.isGM)) {
            return true;
        }
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(TemplatesReach.getTemplatesCenter(template), selectedToken, MeasuredTemplateDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(TemplatesReach.getTemplatesCenter(template), selectedToken, MeasuredTemplateDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.templatesNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
