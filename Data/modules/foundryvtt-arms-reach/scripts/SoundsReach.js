...
export const SoundsReach = {
    globalInteractionDistance: function (selectedToken, sound, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(SoundsReach.getSoundsCenter(sound), selectedToken, AmbientSoundDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(SoundsReach.getSoundsCenter(sound), selectedToken, AmbientSoundDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.soundsNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
