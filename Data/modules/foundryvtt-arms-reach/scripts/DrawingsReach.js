...
export const DrawingsReach = {
    globalInteractionDistance: function (selectedToken, drawing, maxDistance, useGrid, userId) {
            ...
            if (!game.user?.isGM ||
                ...
                if (!selectedToken) {
                    ...
                    if (game.settings.get(CONSTANTS.MODULE_NAME, "globalInteractionDistance") > 0 || useGrid) {
                        ...
                        const dist = computeDistanceBetweenCoordinates(DrawingsReach.getDrawingsCenter(drawing), selectedToken, DrawingDocument.documentName, true);
                        ...
                        const dist = computeDistanceBetweenCoordinates(DrawingsReach.getDrawingsCenter(drawing), selectedToken, DrawingDocument.documentName, false);
                        isNotNearEnough = dist > maxDist;
                    }
                    if (isNotNearEnough) {
                        const tokenName = getCharacterName(selectedToken);
                        if (tokenName) {
                            interactionFailNotification(i18nFormat(`${CONSTANTS.MODULE_NAME}.drawingsNotInReachFor`, { tokenName: tokenName }));
                        }
                        else {
