...
export class DAESystemDND5E extends CONFIG.DAE.systemClass {
    ...
    static modifySpecials(actorType, specials, characterSpec) {
        ...
        specials["system.traits.di.value"] = ["",];
    ...
    static readyActions() {
        ...
        if (game.modules.get("midi-qol")?.active) {
            ...
            Object.keys(CONFIG.DND5E.skills).forEach(skillId => {
                daeSpecialDurations[`isSkill.${skillId}`] = `${i18n("dae.isRollBase")} ${i18n("dae.isSkillDetail")} ${CONFIG.DND5E.skills[skillId].label}`;
            });
        }
    ...
    static configureLists(daeConfig) {
        ...
        this.traitList = duplicate(CONFIG.DND5E.damageTypes);
        ...
        Object.keys(this.traitList).forEach(type => {
            this.traitList[`-${type}`] = `-${CONFIG.DND5E.damageTypes[type]}`;
        });
                ...
                for (let { type, list } of profs) {
                    ...
                    if (ids !== undefined) {
                        ...
                        for (const [key, id] of Object.entries(ids)) {
                            const item = globalThis.dnd5e.applications.ProficiencySelector.getBaseItem(id, { indexOnly: true });
...
function determineSuppression() {
    ...
    if (this.parent) {
        ...
        const statusId = (this.label ?? "no effect").toLocaleLowerCase();
...
function preUpdateItemHook(candidate, updates, options, user) {
    ...
    try {
        ...
        for (let effect of candidate.effects) {
            if (!effect.transfer || effect.origin !== candidate.uuid)
                continue;
            for (let change of effect.changes) {
                if (isSuppressed) {
                    switch (change.key) {
                        case "macro.CE":
                            ...
                            removeConvenientEffectsChange(change.value, actor.uuid, effect.origin, lastArg);
                            break;
                        case "macro.CUB":
                    ...
                    switch (change.key) {
                        case "macro.CE":
                            const lastArg = prepareLastArgData(effect, actor);
                            addConvenientEffectsChange(change.value, actor.uuid, effect.origin, {}, lastArg);
                            break;
                        case "macro.CUB":
                        ...
                        case "macro.createItem":
                            //@ts-expect-error
                            const actorEffect = actor.effects.find(ef => ef.origin === candidate.uuid && ef.flags?.dae?.transfer);
                            addCreateItemChange(change.value, actor, actorEffect);
                            break;
                        default:
                    }
                }
            ...
            if (effect.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                actionQueue.add(daeMacro, isSuppressed ? "off" : "on", actor, effect, actor.getRollData());
        }
    }
    catch (err) {
