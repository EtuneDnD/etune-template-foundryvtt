...
export class DAEActiveEffectConfig extends ActiveEffectConfig {
    ...
    get title() {
        ...
        return `${i18n("EFFECT.ConfigTitle")}: ${this.object.label}` + suffix;
    }
    get id() {
    ...
    async getData(options) {
        ...
        if (game.modules.get("dfreds-convenient-effects")?.active && !getProperty(this.object, "flags.convenientDescription")) {
            setProperty(this.object, "flags.convenientDescription", "test");
            setProperty(data.effect, "flags.convenientDescription", "test");
        }
        return data;
    }
    _keySelected(event) {
    ...
    async _addEffectChange() {
        const idx = this.object.changes.length;
        return this.submit({
            preventClose: true, updateData: {
                //@ts-ignore
    ...
    _getSubmitData(updateData = {}) {
        //@ts-ignore _getSubmitData
        // const data = super._getSubmitData(updateData); TODO revisit when this is fixed
        const fd = new FormDataExtended(this.form, { editors: this.editors });
        let data = expandObject(fd.object);
        if (updateData)
            mergeObject(data, updateData);
