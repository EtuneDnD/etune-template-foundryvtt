...
*/
export function addTokenMagicChange(actor, change, tokens) {
...
export function removeTokenMagicChange(actor, change, tokens) {
    const tokenMagic = globalThis.TokenMagic;
    if (!tokenMagic)
        return;
    for (let token of tokens) {
...
export function addConvenientEffectsChange(effectName, uuid, origin, metaData = {}) {
    const ceInterface = game?.dfreds?.effectInterface;
...
export function prepareLastArgData(effectData, actor, lastArgOptions = {}) {
...
export function _onDeleteActiveEffect(...args) {
    ...
    try {
        ...
        if (effect.data.origin) {
            let origin = DAEfromUuid(effect.data.origin);
...
export async function getMacro({ change, name }, itemData, effectData) {
    if (change.key.includes("macro.execute")) {
        ...
        let macroCommand = itemData?.flags.itemacro?.macro.command ?? itemData?.flags.itemacro?.macro.data.command;
        if (change.macro)
            macroCommand = change.macro.macroCommand;
        if (!itemData)
            itemData = getProperty(effectData.flags, "dae.itemData");
        // 1. Try and get item data to look for the command in.
        if (!macroCommand)
        ...
        if (!macroCommand && !itemData) { // we never got an item do a last ditch attempt
            ...
            macroCommand = itemData?.flags.itemacro?.macro.command ?? itemData?.flags.itemacro?.macro.data.command;
        }
    ...
    if (updates.effects) { // item effects have changed - update transferred effects
        ...
        if (deletions.length > 0) {
          candidate.parent.deleteEmbeddedDocuments("ActiveEffect", deletions).then(() => {
            if (additions.length > 0) candidate.parent.createEmbeddedDocuments("ActiveEffect", additions);
          });
