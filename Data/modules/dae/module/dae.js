...
*/
//@ts-ignore
export function addCreateItemChange(itemDetails, actor, effect) {
    actionQueue.add(socketlibSocket.executeAsGM.bind(socketlibSocket), "createActorItem", { uuid: actor.uuid, itemDetails: itemDetails, effectUuid: effect.uuid });
}
//@ts-ignore
export function removeCreateItemChange(itemId, actor, effect) {
    let [uuid, option] = itemId.split(",").map(s => s.trim());
    if (option === "permanent")
        return; // don't delete permanent items
    if ((effect.flags?.dae?.itemsToDelete ?? []).length === 0)
        return;
    actionQueue.add(socketlibSocket.executeAsGM.bind(socketlibSocket), "removeActorItem", { uuid: actor.uuid, itemUuid: itemId, itemUuids: effect.flags?.dae?.itemsToDelete });
}
...
export function removeConvenientEffectsChange(effectName, uuid, origin, metaData = {}) {
...
export function addConvenientEffectsChange(effectName, uuid, origin, context, CEmetaData = {}) {
    const ceInterface = game?.dfreds?.effectInterface;
    // actionQueue.add(ceInterface.addEffect.bind(ceInterface), { effectName, uuid, origin, metaData })
    let ceEffect = game.dfreds.effects.all.find(e => e.name === effectName);
    if (!ceEffect)
        return;
    let effectData = mergeObject(ceEffect.convertToObject(), context.metaData);
    game.dfreds.effectInterface?.addEffectWith({ effectData, uuid, origin: origin, CEmetaData });
}
export function addCubChange(conditionName, tokens, options = {}) {
...
export function removeCubChange(conditionName, tokens, options = {}) {
    const cubInterface = game?.cub;
    if (cubInterface?.enhancedConditions.supported)
            ...
            try {
                for (let change of effect.changes) {
                    ...
                    if (ceActive && change.key === "macro.CE") {
                        ...
                        addConvenientEffectsChange(change.value, actor.uuid, effect.origin, context, lastArg);
                    }
                    if (change.key === "macro.createItem") {
                        addCreateItemChange(change.value, actor, effect);
                    }
                    const tokenMagic = globalThis.TokenMagic;
                ...
                if (effect.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                    actionQueue.add(daeMacro, "on", parent, effect.toObject(false), {}); // TODO revisit to see if passing the effect is ok
            }
            catch (err) {
...
function preCreateActiveEffectHook(candidate, data, options, user) {
    ...
    if (getProperty(candidate, "flags.dae.stackable") === undefined)
        candidate.updateSource({ "flags.dae.stackable": "multi" });
    const stackable = getProperty(candidate, "flags.dae.stackable");
    if (getProperty(candidate, "flags.dae.transfer") && ["noneName", "none"].includes(stackable)) {
        ...
        const hasExisting = parent.effects.filter(ef => {
            switch (stackable) {
                case "noneName":
                    return ef.label === candidate.label;
                case "none":
                    return candidate.origin && ef.origin === candidate.origin;
            }
        });
    ...
    if (getProperty(candidate, "flags.dae.stackable") !== "noneName")
        return true;
    if (actor.effects.contents.some(ef => ef.label === candidate.label)) {
        return false;
    }
    return true;
}
async function _preCreateActiveEffect(wrapped, ...args) {
    ...
    try {
        ...
        if (!parent || !(parent instanceof CONFIG.Actor.documentClass))
            return wrapped(...args);
        let updates = { "flags.dae.transfer": data.transfer === true ? true : false };
        //@ts-ignore documentClass 
        if (this.flags?.dae?.durationExpression && parent instanceof CONFIG.Actor.documentClass) {
            let sourceActor = parent;
            if (!data.transfer) {
                const thing = await fromUuid(this.origin);
                //@ts-ignore
                if (thing?.actor)
                    sourceActor = thing.actor;
            }
            //@ts-ignore roll(argument)
            const theDuration = await new Roll(`${this.flags.dae.durationExpression}`, sourceActor.getRollData()).roll({ async: true });
        ...
        console;
        let result = wrapped(data, options, user);
        return result;
    }
}
async function evalInline(expression, actor, effect) {
    try {
        console.warn("Doing inlinve eval", expression);
        //@ts-ignore replaceAll
        expression = expression.replaceAll("@data.", "@");
        //@ts-ignore evaluate
        const roll = await (new Roll(expression, actor?.getRollData())).evaluate({ async: true });
        if (showInline) {
            roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor }), flavor: `${effect.label} ${expression}`, chatMessage: true });
        }
        return `${roll.total}`;
    }
    catch (err) {
            ...
            if (effect.changes) {
                ...
                if (changes.disabled === true) {
                    for (let change of effect.changes) {
                        ...
                        if (change.key === "macro.createItem") {
                            removeCreateItemChange(change.value, parent, effect);
                        }
                    }
                    if (effect.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                        actionQueue.add(daeMacro, "off", parent, effect.toObject(false), {});
                }
                    ...
                    for (let change of effect.changes) {
                        ...
                        if (change.key === "macro.createItem") {
                            addCreateItemChange(change.value, parent, effect);
                        }
                    }
                    if (effect.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                        actionQueue.add(daeMacro, "on", parent, effect.toObject(false), {});
                }
            }
        }
        catch (err) {
...
export async function _preUpdateActiveEffect(wrapped, updates, options, user) {
    ...
    try {
        ...
        console.warn(`dae | update active effect Actor ${parent.name}, Effect ${this.label}`, updates, err);
    }
    finally {
        return wrapped(updates, options, user);
    }
    ...
    let changesLoop = async () => {
        try {
            ...
            if (effect.changes) {
                ...
                if (effect.changes.some(change => change.key.startsWith("macro.execute") || change.key.startsWith("macro.itemMacro") || change.key.startsWith("macro.actorUpdate")))
                    actionQueue.add(daeMacro, "off", actor, effect.toObject(false), options);
                if (entityToDelete)
...
export async function daeMacro(action, actor, effectData, lastArgOptions = {}) {
    ...
    for (let change of effectData.changes) {
        try {
            ...
            const theChange = await evalArgs({ item: null, effectData, context, actor, change, doRolls: true });
            let args = [];
...
export async function getMacro({ change, name }, item, effectData) {
    if (change.key.includes("macro.execute")) {
        ...
        if (!macroCommand) {
            ...
            error(`No macro found for item ${item?.name}`);
        }
...
export async function applyNonTransferEffects(item, activate, targets, options = { whisper: false, spellLevel: 0, damageTotal: null, itemCardId: null, critical: false, fumble: false, tokenId: undefined, effectsToApply: [], removeMatchLabel: false, toggleEffect: false, selfEffects: "none" }) {
    ...
    for (let [aeIndex, activeEffectData] of appliedEffects.entries()) {
        for (let [changeIndex, change] of activeEffectData.changes.entries()) {
            ...
            /* let newChange = await evalArgs({ item: item, effectData: activeEffectData, context: rollData,
              actor: item.actor, change, spellLevel: options.spellLevel, damageTotal: options.damageTotal,
              doRolls, critical: options.critical, fumble: options.fumble,
              itemCardId: options.itemCardId, whisper: options.whisper }) */
...
function updateItemEffects(candidate, updates, options, user) {
    ...
    if (updates.effects) { // item effects have changed - update transferred effects
        ...
        let additions = candidate.effects.filter(aef => {
            const isTransfer = aef.flags?.dae?.transfer === true || aef.transfer === true;
            setProperty(aef.flags, "dae.transfer", isTransfer);
            return isTransfer;
        });
        additions = additions.map(ef => ef.toObject(false)); // TODO v10 try toObjct
...
//TODO find out why this is here - does not seem to make any sense 
export function preCreateItemHook(candidate, data, options, user) {
    return true;
    if (!candidate.isOwned)
        return true;
    if (data.effects) {
        candidate.effects.delete();
        candidate.updateSource({ "effects": data.effects });
    }
    return true;
}
export /*async*/ function preUpdateActor(candidate, updates, options, user) {
    try {
        ...
        for (let onUpdate of (getProperty(candidate, "flags.dae.onUpdateTarget") ?? [])) {
            ...
            if (onUpdate.filter.startsWith("data.")) {
                console.error(`dae | onUpdateTarget ${onUpdate.filter} is deprecated use ${onUpdate.filter.replace("data.", "system.")}`);
    ...
    ${macroText}
                ...
                // await fn.call(this, { speaker, actor: candidate, token: undefined, character, item: originItem, args });
                fn.call(this, { speaker, actor: candidate, token: undefined, character, item: originItem, args });
            }
            catch (err) {
