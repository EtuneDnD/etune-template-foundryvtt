...
class DataConverterClass extends DataConverter {
	...
	static async pGetClassItem (cls, opts) {
		...
		const effectsSideTuples = await this._pGetEffectsSideLoadedTuples({ent: cls, img});
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importClass", "ownership")};
	...
	static _getClassSubclassFlags ({cls, sc, filterValues, proficiencyImportMode, isActorItem, spellSlotLevelSelection}) {
		...
		if (isActorItem) out[SharedConsts.MODULE_NAME].isDirectImport = true;
	...
	static async pGetSubclassItem (cls, sc, opts) {
		...
		const effectsSideTuples = await this._pGetEffectsSideLoadedTuples({ent: sc, img}, {propOpts: "_SIDE_LOAD_OPTS_SUBCLASS"});
		effectsSideTuples.forEach(({effect, effectRaw}) => DataConverter.mutEffectDisabledTransfer(effect, "importClass", UtilActiveEffects.getDisabledTransferHintsSideData(effectRaw)));
	...
	static _getExistingPreparedSpellsEffect ({actor}) {
		...
		return actor.effects.contents.find(it => (it.label || "").toLowerCase().trim() === "prepared spells");
	}

	static async _pGetPreparedSpellsEffect ({cls, sc, actorId, itemId, existing}) {
		...
		// if (!UtilCompat.isTidy5eSheetActive()) return null;
		...
		return UtilActiveEffects.getGenericEffect({
			key: "system.details.maxPreparedSpells",
			...
			icon: await Vetools.pOptionallySaveImageToServerAndGetUrl(await this.pGetClassImagePath(cls)),
			disabled: false,
