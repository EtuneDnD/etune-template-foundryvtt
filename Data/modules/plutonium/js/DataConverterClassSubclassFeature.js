...
class DataConverterClassSubclassFeature extends DataConverterFeature {
	...
	static async pGetClassSubclassFeatureItem (feature, opts) {
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importClassSubclassFeature", "ownership")};
	...
	static async pGetClassSubclassFeatureItemEffectTuples (actor, feature, sheetItem, {img} = {}) {
		const out = [];
	...
	static _getClassSubclassFeatureFlags (feature, type, opts) {
		...
		if (opts.isAddDataFlags) {
			out[SharedConsts.MODULE_NAME].propDroppable = prop;
