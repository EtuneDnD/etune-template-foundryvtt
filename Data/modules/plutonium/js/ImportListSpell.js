...
class ImportListSpell extends ImportList {
	...
	static async _pGetSpellScrollData (spellData) {
		// FIXME(v12) remove this ".data" bodge
		...
		$html.find(`[name="btn-run"]`).click(async () => {
			const enhancedSpells = this._list.items.map(it => {
				...
				if (hasIsPrep || hasPrepMode) {
					out.foundrySystem = {};
					if (hasIsPrep) out.foundrySystem["preparation.prepared"] = !!Number(isPrepVal);
					if (hasPrepMode) out.foundrySystem["preparation.mode"] = prepModeVal;
				}
