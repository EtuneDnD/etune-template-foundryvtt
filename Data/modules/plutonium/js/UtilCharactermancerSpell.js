...
class Charactermancer_Spell extends BaseComponent {
	...
	_getExistingSpellLookup () {
		...
		[...spItems]
			...
			.sort((a, b) => {
				const flagsA = a.flags?.[SharedConsts.MODULE_NAME];
				const flagsB = b.flags?.[SharedConsts.MODULE_NAME];
			...
			.forEach(spItem => {
				...
				const flags = spItem.flags?.[SharedConsts.MODULE_NAME];
				...
				const isItemAlwaysPrepared = spItem.system.preparation?.mode === "always"; // Trust the item data on this
