...
import {UtilFolderPathBuilder} from "./FolderPathBuilder.js";
...
class ImportListAdventureBook extends ImportList {
	...
	async _pHandleRunButtonClick_pDoMainImport ({preloadedTagMetas, runInfo}) {
		...
		const handlers = {
			string: (str) => {
				...
				for (let i = 0; i < len; ++i) {
					...
					// e.g. `@UUID[Actor.myId]{myDisplayText}`
		...
		const folderIdMetaRoot = await this._pImportEntry_pGetFolderIdMeta(
			...
			},
			{
				sorting: "m",
				isAddOwnership: true,
				// As this is the root folder inside which all other folders/journal entries will be placed, avoid creating
				//   a journal entry.
		...
		for (const wrappedJournalData of wrappedJournalDatas) {
			const task = new Util.Task(
				...
				() => this._pImportWrappedJournalData({finalizeLinkLookup, entryIdToJournalId, folderIdMetaRoot, wrappedJournalData}),
			);
			taskList.push(task);
		}
	...
	async _pImportWrappedJournalData ({finalizeLinkLookup, entryIdToJournalId, folderIdMetaRoot, wrappedJournalData}) {
		...
		// region Legacy duplicates
		const duplicateMetaOpts = {
			name: journalEntry.name,
			[SharedConsts.MODULE_NAME]: {
				source: journalEntry.flags[SharedConsts.MODULE_NAME].source,
			},
		};

		// If we find a legacy-style freestanding journal entry, fabricate a page, and fast exit
		...
		if (duplicateMeta?.isSkip || duplicateMeta?.isOverwrite) {
			(duplicateMeta.existing?.pages || []).forEach(page => {
				const entryIds = page?.flags?.[SharedConsts.MODULE_NAME]?.entryIds || [];
		...
		let folderIdMeta = folderIdMetaRoot;
		// If we have a parent journal entry, then we are therefore a "leaf", so import as pages to the parent journal.
		//   Otherwise, create a leaf journal entry (plus parent folders, if required).
		if (!folderIdMetaRoot.parentDocumentId) {
			folderIdMeta = await UtilFolderPathBuilder.pGetCreateFolderIdMeta({
				folderType: this.constructor.FOLDER_TYPE,
				folderIdRoot: folderIdMetaRoot.folderId,
				folderNames,
				sorting: "m",
			});
		}
		...
		if (!folderIdMeta.parentDocumentId) {
			journalEntryDoc = await JournalEntry.create(journalEntry, {renderSheet: false, temporary: false});
			journalPageDoc = journalEntryDoc.pages[0];
		} else {
			journalEntryDoc = game.journal.get(folderIdMeta.parentDocumentId);
		...
		const flags = journalPageDoc?.flags?.[SharedConsts.MODULE_NAME];
		if (flags) {
		...
		const entryIds = journalPageDoc?.flags?.[SharedConsts.MODULE_NAME]?.entryIds || [];
		entryIds.forEach(id => entryIdToJournalId[id] = {
			journalEntryDoc,
			journalPageDoc,
		});
