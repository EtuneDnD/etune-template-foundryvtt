...
class DataConverterItem extends DataConverter {
	...
	static async _pGetActionWeaponDetails_initCaches () {
		...
		try {
			...
			for (const item of items) {
				...
				cpy._fvttImage = await this._pGetImagePath(cpy, {fluff: procFluff, propCompendium: "item"});
	...
	static async pGetItemItem (item, opts) {
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importItem", "ownership")};
	...
	static _getItemFlags (item, opts) {
		...
		if (opts.isAddDataFlags) {
			out[SharedConsts.MODULE_NAME].propDroppable = "item";
