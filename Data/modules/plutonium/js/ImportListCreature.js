...
class ImportListCreature extends ImportListActor {
	...
	async _pImportEntry_pFillItems (mon, act, dataBuilderOpts, importOpts) {
		...
		await this._pImportEntry_pFillItems_pActions(mon, act, dataBuilderOpts);
		...
		await this._pImportEntry_pFillItems_pTraitsReactionsLegendaries(mon, act, dataBuilderOpts, {prop: "bonus"}, {
			activationType: "bonus",
			activationCost: 1,
		});
		await this._pImportEntry_pFillItems_pTraitsReactionsLegendaries(mon, act, dataBuilderOpts, {prop: "reaction"}, {
		...
		await this._pImportEntry_pFillItems_pLegendaryHeader(mon, act, dataBuilderOpts, legendaryItems);
		...
		await this._pImportEntry_pFillItems_pMythicHeader(mon, act, dataBuilderOpts, mythicItems);
		await this._pImportEntry_pFillItems_pOtherHeaders(mon, act, dataBuilderOpts, mythicItems);
		await this._pImportEntry_pFillItems_pLairActions(mon, act, dataBuilderOpts);
		await this._pImportEntry_pFillItems_pRegionalEffects(mon, act, dataBuilderOpts);
		await this._pImportEntry_pFillItems_pVariants(mon, act, dataBuilderOpts);

		await this._pImportEntry_pFillItems_pInventory(mon, act, dataBuilderOpts);

		// If importing a creature as a player character, add a fake class to provide spellcasting slots/proficiency bonus
		...
		await this._pImportEntry_pFillItems_pSpellcasting(mon, act, isTemporary, dataBuilderOpts);
	...
	async _pImportEntry_pFillItems_pTraitsReactionsLegendaries (mon, act, dataBuilderOpts, opts, optsFoundryData) {
		...
		for (let entry of mon[opts.prop]) {
			...
			// Always generate Ids, as we expect to link legendary actions back to a single "legendary actions" parent
	...
	static _getCasterProgressionFromEntry (casting, level, spellClass) {
		if (casting.spells) {
			...
			// Attempt to match the max spell level with a known caster progression
			const casterTypeVe = Object.entries(UtilDataConverter.CASTER_TYPE_TO_PROGRESSION)
				.filter(([casterType]) => casterType !== "pact")
				// Prefer "half-caster" over "artificer", as it is more common (unless we're an artificer)
				...
				.find(([, progressions]) => {
					...
					// If we have the max level spell for this level and no higher spell level, it's a match
...
ImportListCreature.ImportEntryOpts = class extends ImportListActor.ImportEntryOpts {
	...
	getExistingItemItemIndex (itemHash) {
		return this.items.findIndex(it => {
			if (!it?.flags?.[SharedConsts.MODULE_NAME]) return false;
			return it.flags[SharedConsts.MODULE_NAME].page === UrlUtil.PG_ITEMS
				&& it.flags[SharedConsts.MODULE_NAME].hash === itemHash;
		});
	}
};
...
ImportListCreature.ImportCustomizer = class extends ImportCustomizer {
	...
	_activateListeners_bindControls ({$html, $wrpBtnsSort}) {
		...
		$html.find(`[name="btn-run"]`).click(async () => {
			const scaledMons = this._list.items
				.pMap(async li => {
					let mon = this._dataList[li.ix];

					mon = await this._activateListeners_bindControls_pVariant({li, mon});
					mon = await this._activateListeners_bindControls_pScale({li, mon});
					mon = await this._activateListeners_bindControls_pRename({li, mon});
					mon = await this._activateListeners_bindControls_pToken({li, mon});
					mon = await this._activateListeners_bindControls_pIsCharacter({li, mon});
	...
	async _activateListeners_bindControls_pVariant ({li, mon}) {
		const ixVariant = li.data.$selVariant.length
			? Number(li.data.$selVariant.val()) === -1 ? null : Number(li.data.$selVariant.val())
			: null;

		if (ixVariant == null) return mon;

		const variants = this._getAvailableVariants(mon);

		const variant = variants[ixVariant];
		if (variant.fnApplyVariant) return variant.fnApplyVariant(mon);
		else return variant;
	}

	async _activateListeners_bindControls_pScale ({li, mon}) {
		...
		if (targetCr != null && targetCr !== Parser.crToNumber(mon.cr)) return ScaleCreature.scale(mon, targetCr);
		else if (targetSpellLevel != null) return ScaleSpellSummonedCreature.scale(mon, targetSpellLevel);
		else if (targetClassLevel != null) return ScaleClassSummonedCreature.scale(mon, targetClassLevel);

		return mon;
	}

	async _activateListeners_bindControls_pRename ({li, mon}) {
		const rename = li.data.$iptRename.val().trim();
		if (!rename) return mon;

		const renameShort = li.data.$iptRenameShort.val().trim();
		const isNamedCreature = li.data.$cbIsNamedCreature.prop("checked");
		return ImportListCreature.ImportCustomizer._pGetRenamedMon(mon, {newName: rename, newNameShort: renameShort, isNamedCreature});
	}

	async _activateListeners_bindControls_pToken ({li, mon}) {
		const ixToken = Number(li.data.$selToken.val()) === -1 ? null : Number(li.data.$selToken.val());
		if (ixToken == null) return mon;
	...
	async _activateListeners_bindControls_pIsCharacter ({li, mon}) {
		const isCharacter = li.data.$cbIsCharacter.prop("checked");
		if (!isCharacter) return mon;

		mon.foundryType = "character";
