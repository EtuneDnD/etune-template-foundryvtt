...
class UtilActors {
	...
	static isImporterTempActor (actor) { return !!MiscUtil.get(actor, "flags", SharedConsts.MODULE_NAME, "isImporterTempActor"); }
	...
	static hasActorSpellPointSlotEffect ({actor}) {
		return (actor?.effects || []).some(it => it.flags[SharedConsts.MODULE_NAME]?.["isSpellPointsSlotUnlocker"]);
	}
	...
	static async pLinkTempUuids ({actor}) {
		const SENTINEL = `__${SharedConsts.MODULE_NAME_FAKE}_REPLACE_TARGET__`;

		const reUuid = new RegExp(`(?<prefix>@UUID\\[[^\\]]+\\.)temp-${SharedConsts.MODULE_NAME_FAKE}-(?<packed>[^.\\]]+)(?<suffix>](?:\\{[^}]+})?)`, "g");
				...
				const nxtDesc = desc
					.replace(reUuid, (...m) => {
						...
						try {
							...
							const matchedItem = actor.items.find(it => it.flags?.[SharedConsts.MODULE_NAME]?.page === page && it.flags?.[SharedConsts.MODULE_NAME]?.source === source && it.flags?.[SharedConsts.MODULE_NAME]?.hash === hash);
					...
					// FIXME crappy hack; restructure to remove the need for this (see also: `UtilCharactermancerClass`)
...
class SpellPsiPointsItemBuilder {
	...
	static getItem ({actor}) {
		...
		return actor.items.contents.find(it => it.flags?.[SharedConsts.MODULE_NAME]?.type === this._FLAG_TYPE);
	}
