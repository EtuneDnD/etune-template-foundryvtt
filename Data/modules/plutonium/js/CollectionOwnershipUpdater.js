...
class CollectionOwnershipUpdater extends BaseCollectionTool {
	...
	async _pHandleBtnClick_run () {
		...
		const tasks = toSaves.map(toSave => new Util.Task(`${toSave.entityMeta.path}${toSave.entityMeta.name}`, () => this._pUpdateItemOwnership(toSave)));
	...
	async _pUpdateItemOwnership (toSave) {
		...
		(toSave.metasPlayerUpdates || []).forEach(meta => {
			...
			// "-1" is effectively "delete this ownership," since Foundry screeches if we _actually_ pass -1.
			// The same strategy is used in `DocumentOwnershipConfig._updateObject`.
...
export {CollectionOwnershipUpdater};
