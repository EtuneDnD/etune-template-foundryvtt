...
class UtilCompendium {
	 ...
	 * @param [opts.deepKeys] A list of keys (e.g. `"system.save.ability"` for data which should be cached when loading the compendium, to enable later lookups.
	...
	static async _pGetPlutoniumCompendiumId_generic ({page, hash, packName}) {
		...
		const match = UtilCompendium._PLUT_CACHES[packName].find(it => it.flags?.[SharedConsts.MODULE_NAME]?.page === page && it.flags?.[SharedConsts.MODULE_NAME]?.hash === hash);
