...
class DataConverterOptionalfeature extends DataConverterFeature {
	...
	static async _pGetOptionalFeatureItem_fromSrd (optFeature, opts) {
		...
		const effectsSideTuples = (await this._pGetEffectsSideLoadedTuples({ent: optFeature, img: srdData.img}) || []);
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importOptionalFeature", "ownership")};
	...
	static async _pGetOptionalFeatureItem_other (optFeature, opts) {
		...
		const effectsSideTuples = await this._pGetEffectsSideLoadedTuples({ent: optFeature, img});
		effectsSideTuples.forEach(({effect, effectRaw}) => DataConverter.mutEffectDisabledTransfer(effect, "importOptionalFeature", UtilActiveEffects.getDisabledTransferHintsSideData(effectRaw)));
	...
	static _getOptionalFeatureFlags (optFeature, opts) {
		...
		if (opts.isAddDataFlags) {
			out[SharedConsts.MODULE_NAME].propDroppable = "optionalfeature";
	...
	static async pGetOptionalFeatureItemEffectTuples (actor, optFeature, sheetItem, {img} = {}) {
		const effectsRaw = await this._pGetEffectsRawSideLoaded_(optFeature, this._SIDE_LOAD_OPTS);
		return UtilActiveEffects.getExpandedEffects(effectsRaw || [], {actor, sheetItem, parentName: optFeature.name, img}, {isTuples: true});
	}
