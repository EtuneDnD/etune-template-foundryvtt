...
import {FolderIdMeta, MixinFolderPathBuilder} from "./FolderPathBuilder.js";
...
class ImportList extends MixinHidableApplication(MixinFolderPathBuilder(Application)) {
	...
	/**
	 * Tolerate junk from modules, e.g. Quick Insert.
	 */
	static _isBadlyDroppedCustomUid ({data}) {
		if (!data.uuid) return false;
		return /^@[a-z][a-zA-Z]+\[[^\]]+]$/.test(data.uuid);
	}
	...
	static async _pHandleDropGetImportedDoc_badCustomUidDrop (data, {isTemp = false, requiresDocumentName = null, pack = null} = {}) {
		...
		const entity = await Renderer.hover.pCacheAndGet(pageHover || page, source, hashHover || hash, {isCopy: true});
	...
	static _initCreateSheetItemHook (
		...
		Hooks.on("preCreateItem", (item, itemData, options, itemId) => {
			...
			const flags = itemData.flags?.[SharedConsts.MODULE_NAME];
			...
			this._pGetUseImporterDragDrop({isForce})
				.then(async isUseImporter => {
					...
					try {
						...
						MiscUtil.set(itemData.flags, SharedConsts.MODULE_NAME, "isStandardDragDrop", true);
	 ...
	 * @param [subclassOpts.propsFluff] JSON data properties for this entity's fluff (e.g. `["monsterFluff"]`).
	...
	constructor (applicationOpts, externalData, subclassOpts) {
		...
		this._propsFluff = subclassOpts.propsFluff;
	...
	async pInit () {
		...
		await this._pInit_pFluffLookup();
	}

	async _pInit_pFluffLookup () {
		this._fluffLookup = {};

		const fluff = await this._pInit_pFluffLookup_pGetFluff();
		(this._propsFluff || [])
			.forEach(prop => (fluff[prop] || []).forEach(it => (this._fluffLookup[it.source] = this._fluffLookup[it.source] || {})[it.name] = it));
	}

	async _pInit_pFluffLookup_pGetFluff () { return {}; }

	_getLookupFluff ({ent}) { return ent.fluff || MiscUtil.get(this._fluffLookup, ent.source, ent.name); }
	...
	/**
	 * @return {Promise<ImportSummary>}
	 */
	async _pImportEntry (ent, importOpts) { throw new Error(`Unimplemented!`); }
	 ...
	 * @param [opts.isFoldersOnly]
	 * @param [opts.folderType]
	 */
	async _pImportEntry_pGetFolderIdMeta (entry, opts) {
		...
		else if (opts.isAddOwnership && Config.has(this._configGroup, "ownership")) defaultOwnership = Config.get(this._configGroup, "ownership");
	 ...
	 * @param [opts.source]
	 * @param [opts.entity]
	 * @param [opts.flags] N.b.: only implemented for somedocument types. // TODO(Future) Implement for actors/items as required.
	...
	/**
	 * FIXME(Future) We assume the parent is a journal entry
	 * @param opts
	 * @param opts.parent
	 * @param opts.children
	 * @param [opts.gameProp] Game prop override
	...
	/** Find all embedded documents on a document, and delete them. */
	...
	async _pImportEntry_pImportToDirectoryGeneric (toImport, importOpts, dataOpts = {}, {docData = null, isSkipDuplicateHandling = false} = {}) {
		...
		const duplicateMeta = isSkipDuplicateHandling
			? null
			: this._getDuplicateMeta({name: docData.name, source: MiscUtil.get(docData, "system", "source"), importOpts});
		...
		// FIXME(Future) improve importing journal entries to compendiums--at present, we import them all as standalone.
		//   Instead, allow the user to choose a parent journal entry name (using e.g. a limited version of the folder
		//   path builder UI), and match the import-to-directory flow, below.
		if (this._pack) {
	...
	async _pImportEntry_pImportToDirectoryGeneric_toDirectory (
		...
		const folderIdMeta = importOpts.folderId !== undefined
			? new FolderIdMeta({folderId: importOpts.folderId})
			: await this._pImportEntry_pGetFolderIdMeta(
				toImport,
				{
					isAddOwnership: true,
					defaultOwnership: importOpts.defaultOwnership,
	...
	async _pImportEntry_pImportToDirectoryGeneric_toDirectorySubEntities (
		...
		// `_id`s are set during duplicate check
