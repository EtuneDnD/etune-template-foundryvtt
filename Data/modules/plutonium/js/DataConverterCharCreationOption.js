...
class DataConverterCharCreationOption extends DataConverterFeature {
	...
	static async _pGetCharCreationOptionItem (ent, opts) {
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importCharCreationOption", "ownership")};
	...
	static _getCharCreationOptionFlags (ent, opts) {
		...
		if (opts.isAddDataFlags) {
			out[SharedConsts.MODULE_NAME].propDroppable = "charoption";
	...
	static async pGetCharCreationOptionItemEffectTuples (actor, ent, sheetItem, {img} = {}) {
		const effectsRaw = await this._pGetEffectsRawSideLoaded_(ent, this._SIDE_LOAD_OPTS);
		return UtilActiveEffects.getExpandedEffects(effectsRaw || [], {actor, sheetItem, parentName: ent.name, img}, {isTuples: true});
	}
}
