...
class PageFilterClassesFoundry extends PageFilterClassesRaw {
	...
	static _pLoadSubEntries_getMappedWalkerArrayEntry ({it, path, references, actor, isIgnoredLookup, ...opts}) {
		...
		// region FIXME(Future)
		// FIXME this is probably the best way to link parent -> child items, but it's far from the best way to clean
		//   the description of unused links.
		//   We need a way of trimming down e.g. lists of entries to remove any dead/broken links due to un-chosen
		//    options/features. Possible solution: parse the description HTML after replacing all links, remove any
		//    empty text nodes, remove their parents, ... etc.
		//
		//   Some examples of things which use this:
		//   - Sorcerer "Font of Magic" -> "Sorcery Points"/"Flexible Casting"
		//   - Beastheart "Primal Exploits"
		...
		return {
			...
			wrapped: actor
				? `@UUID[Actor.${actor.id}.Item.temp-${SharedConsts.MODULE_NAME_FAKE}-${b64Ident}]{${name}}`
				: `@UUID[Item.temp-${SharedConsts.MODULE_NAME_FAKE}-${b64Ident}]{${name}}`,
			source,
			data: {
	...
	constructor (actor) {
		...
		actor.items
			...
			.forEach(it => {
				...
				const {page, source, hash} = it.flags?.[SharedConsts.MODULE_NAME] || {};
...
class Charactermancer_Class_StartingProficiencies extends BaseComponent {
	...
	static getExistingProficienciesFvttSavingThrows (actor) {
		...
		return Object.entries(MiscUtil.get(actor, "system", "abilities") || {})
