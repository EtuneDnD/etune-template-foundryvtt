...
class ImportListItem extends ImportList {
	...
	async _pImportEntry (item, importOpts) {
		...
		const fluff = this._getLookupFluff({ent: item});
		const opts = {fluff};
	...
	async pImportCurrency (currency, importOpts) {
		...
		const itemData = await DataConverterItem.pGetCurrencyItem(currency, {isAddOwnership: !this._actor});
	...
	static _initCreateSheetItemHook_currency () {
		Hooks.on("preCreateItem", (item, itemData, options, itemId) => {
			...
			const flags = itemData.flags?.[SharedConsts.MODULE_NAME] || itemData.flags?.[SharedConsts.MODULE_NAME];
