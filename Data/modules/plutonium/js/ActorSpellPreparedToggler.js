...
class ActorSpellPreparedToggler extends Application {
		...
		$html.find(`[name="btn-list-save"]`)
			.click(async () => {
				try {
					...
					const existingActorFlags = this._actor.flags?.[SharedConsts.MODULE_NAME];
					const nxtFlags = {...existingActorFlags || {}};
					MiscUtil.set(nxtFlags, this.constructor.name, "saveId", this._activeSaveId);
					await UtilDocuments.pUpdateDocument(this._actor, {flags: {[SharedConsts.MODULE_NAME]: nxtFlags}});
					// endregion
	...
	async _pDoLoadInitialState () {
		...
		const flags = this._actor.flags?.[SharedConsts.MODULE_NAME];
