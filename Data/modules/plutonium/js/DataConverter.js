...
class DataConverter {
	...
	static async _pGetImagePath (ent, {fluff, propCompendium, isAllowCustom = true} = {}) {
		const meta = await this._pGetImagePathMeta_(...arguments);
		return meta.img;
	}

	static async _pGetSaveImagePath (ent, {fluff, propCompendium, isAllowCustom = true} = {}) {
		return Vetools.pOptionallySaveImageToServerAndGetUrl(await this._pGetImagePath(ent, {fluff, propCompendium, isAllowCustom}));
	}

	static async _pGetSaveImagePathMeta (ent, {fluff, propCompendium, isAllowCustom = true} = {}) {
		const meta = this._pGetImagePathMeta_(ent, {fluff, propCompendium, isAllowCustom});
	...
	static async _pGetImagePathMeta_ (ent, {fluff, propCompendium, isAllowCustom = true} = {}) {
				...
				cpyEntries,
				{
					string: (str) => {
						return str
							...
							.replace(/{@hitYourSpellAttack}/gi, () => `{@dice 1d20 + @${SharedConsts.MODULE_NAME_FAKE}.userchar.spellAttackRanged|your spell attack modifier}`)
	...
	static _pGetEntryDescription_getCleanDicePart (str, opts) {
		return str
			// Replace `+ PB`
			.replace(/\bPB\b/gi, `@${SharedConsts.MODULE_NAME_FAKE}.userchar.pb`)
	...
	static getDamageTupleMetas (str, {summonSpellLevel = 0} = {}) {
		...
		const strOut = str
			.replace(/(?:(?<dmgFlat>\d+)|\(?{@(?:dice|damage) (?<dmgDice1>[^|}]+)(?:\|[^}]+)?}(?:\s+[-+]\s+the spell's level)?(?: plus {@(?:dice|damage) (?<dmgDice2>[^|}]+)(?:\|[^}]+)?})?\)?)(?:\s+[-+]\s+[-+a-zA-Z0-9 ]*?)?(?: (?<dmgType>[^ ]+))? damage/gi, (...mDamage) => {
				...
				dmg = dmg.replace(/\bPB\b/gi, `@${SharedConsts.MODULE_NAME_FAKE}.userchar.pb`);
	...
	static async _pGetSpeedEffects (speeds, {actor, actorItem, iconEntity, iconPropCompendium} = {}) {
		...
		const effectWalkSpeed = UtilActiveEffects.getGenericEffect({
			key: `system.attributes.movement.walk`,
			value: speeds?.walk ?? speeds,
		...
		if (typeof speeds === "number") return [effectWalkSpeed];

		const toMap = Object.entries(speeds)
			.filter(([k, v]) => this._SPEED_PROPS_IS_EQUAL_MAP[k] && v === true);

		if (!toMap.length) return [effectWalkSpeed];
			...
			.forEach((found, i) => {
				...
				Object.entries(found)
					...
					.forEach(([prop, v]) => {
						...
						// Prefer existing data over incoming data, as we use "custom" -> "brew" -> "other" ordering above
	...
	// TODO(v10) remove support for `.data`
	// Handle legacy data
	...
	static async _pGetStarSideLoadedFound (ent, {propFromEntity, propFromSideLoaded, found}) {
		...
		// Allow the `foundrySystem` property on the entity to override/compliment anything defined as separate data
