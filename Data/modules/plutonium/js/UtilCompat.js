...
class UtilCompat {
	...
	static MonksLittleDetails = class {
		/**
		 * Based on `MonksLittleDetails.isDefeated` and `Combatant.isDefeated`
		 */
		static isDefeated (token) {
			return (
				(token.combatant && token.isDefeated)
				// Manually check for the defeated status, since using the combatant only works if the token is a combatant(!)
				|| token.actor?.effects.some(it => it.getFlag("core", "statusId") === CONFIG.specialStatusEffects.DEFEATED)
