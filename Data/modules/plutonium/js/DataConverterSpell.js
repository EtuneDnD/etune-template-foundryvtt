...
class DataConverterSpell extends DataConverter {
	...
	static async pGetSpellItem (spell, opts) {
		...
		else if (opts.isAddOwnership) out.ownership = {default: Config.get("importSpell", "ownership")};
		...
		if (replacementData) {
			[
				...
				["system", "save", "scaling"],
				["permission"],
				["flags", SharedConsts.MODULE_NAME],
			].forEach(path => {
	...
	static _getSpellFlags (
		...
		out[SharedConsts.MODULE_NAME].parentClassName = parentClassName;
		out[SharedConsts.MODULE_NAME].parentClassSource = parentClassSource;
		out[SharedConsts.MODULE_NAME].parentSubclassName = parentSubclassName;
		out[SharedConsts.MODULE_NAME].parentSubclassSource = parentSubclassSource;
