...
class LootGeneratorApp extends Application {
	...
	static async patcher_pHandleActorDrop (...args) {
		...
		if (data?.type === "JournalEntryPage") {
			...
			if (doc?.flags?.[SharedConsts.MODULE_NAME]?.type === LootGenOutputFvtt.FLAG_TYPE__LOOT) {
				await LootGeneratorApp.pImportLoot({
					loot: doc.flags[SharedConsts.MODULE_NAME].loot,
					actor: this.actor,
				});
				return true;
			}
...
class LootGenOutputFvtt extends LootGenOutput {
	...
	static async pImportToJournal ({loot}) {
		...
		const existingJournal = game.journal.find(it => it.flags?.[SharedConsts.MODULE_NAME]?.type === LootGenOutputFvtt.FLAG_TYPE__LOOT_BINDER);
