...
class Plutonium {
	...
	static _init_allowPackageOperations () {
		import("./plutonium-backend-addon-package-operations.mjs")
			.catch(() => {
				console.error(`Failed to load Package Operations addon. If you have not installed the Package Operations addon, you should ignore this message.`);
			})
			.then(mod => {
				if (!mod) return;
				Plutonium._MODULE_PACKAGE_OPERATIONS = mod;
			});
	}
	...
	static _init_allowCustomLoginFiles () {
		import("./plutonium-backend-addon-custom-login.mjs")
			.catch(() => {
				console.error(`Failed to load Custom Login addon. If you have not installed the Custom Login addon, you should ignore this message.`);
			})
			.then(mod => {
				if (!mod) return;
				mod.doPatch();
